...
def main_table(a):
    ...
    L = long_head("llrrrrr", cap, "tab:main",
                  r"Design & Method & Active & Precision & Recall & $F_1$ (MCSE) & Test MSE")
    ...
    for si, s in enumerate(SC):
        for i, (m, e) in enumerate(order):
            ...
            L.append(f"{s if i == 0 else ''} & {nm} & {r.active_m:.1f} & "
                     ...
                     f"{r.F1_m:.3f} ({r.F1_se:.3f}) & {r.test_mse_m:.2f} \\\\")
        if si < 2:
            L.append(r"\midrule")
...
def real_table():
    ...
    d = d[d.E.isna() | d.E.isin([1, 3, 20])]
    cap = (r"Fixed synthetic benchmark ($n=800$, $p=600$, $K=3$). The original generator "
           r"and true coefficients were not retained. Agreement is with the pooled minimum-MSE "
    ...
    L[2] = L[2].replace("Upload includes validation scalars and all five candidate fits for P; ADMM-CV costs are not measured.",
                         "Selected epoch counts are shown; the complete grid and costs, including all five P candidates, are supplied in the result CSV.")
