...
\caption{Fixed synthetic benchmark ($n=800$, $p=600$, $K=3$). The original generator and true coefficients were not supplied. Agreement is with the pooled minimum-MSE Lasso support (\Srefactive\ variables), not with ground truth. $F$ is the common evaluation objective. Selected epoch counts are shown; the complete grid and costs, including all five P candidates, are supplied in \texttt{code/real\_data\_results.csv}.}\label{tab:real}\\
\toprule
