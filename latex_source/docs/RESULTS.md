...
The fixed synthetic node benchmark has unknown true support, so its F1 denotes agreement with the pooled minimum-MSE reference. The orthogonal illustration in Table S3 instead has a known zero coefficient vector and compares Monte Carlo null-selection frequencies with the exact formula.
