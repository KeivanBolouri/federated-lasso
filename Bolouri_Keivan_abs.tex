...
\begin{center}
{\large\bfseries How Many Local Epochs? Communication Cost and Variable\\
Selection in Federated Lasso for Multi-Site Studies}\\[0.55em]
Keivan Bolouri\\[0.15em]
{\footnotesize Department of Statistics, University of California, Irvine
\quad\textbullet\quad
\texttt{keivan.bolouri.78@gmail.com}}
\end{center}
...
sponsor controls. Federated learning shares model parameters rather than patient records. When the goal is
...
\noindent\textbf{Setup.}
We evaluate a synchronous federated coordinate-descent Lasso using the publicly available
STAT~102B high-dimensional regression dataset obtained from
\texttt{github.com/KeivanBolouri/federated-lasso}.
After preprocessing, the data contain $n=800$ observations and $p=600$ predictors and are
computationally partitioned into $K=3$ disjoint sites with sample sizes $m_j=400,240,160$.
Each site selects its regularization parameter independently. Because the true coefficient support is
unknown, a centralized Lasso fitted to the pooled sample serves as the reference throughout.
Each site then runs $E$ full coordinate-descent passes per round, and the server aggregates by
sample-size weight; no row-level data are exchanged during federated fitting. We sweep $E\in\{1,2,3,5,8,10,15,20\}$,
recording communication rounds, upload cost, and agreement between the federated active set and
that reference (197 active predictors).
...
against 25.8906 at $E=1$), missing the $E=2$--$3$ region entirely. The objective and the selection metrics
disagreed about which schedule was best, suggesting that the penalized objective alone may be insufficient
for choosing $E$ when support stability is important.
...
above is motivation, not demonstrated performance. Because the true support is unknown, the pooled
Lasso is an estimated reference and the metrics measure agreement rather than correctness. The recall
pattern is consistent with client drift, though drift was not measured directly, and no comparison is made
against FedProx, SCAFFOLD, or ADMM-based distributed Lasso --- replication with known true support,
direct drift measurement, and those comparisons are in progress.
